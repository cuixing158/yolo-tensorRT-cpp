...
void Yolo::parseConfigBlocks()
{
    for (auto block : m_configBlocks)
    {
        if (block.at("type") == "net")
        {
            ...
            m_InputH = std::stoul(block.at("height"));// 转换 string to unsigned long
            m_InputW = std::stoul(block.at("width"));
            m_InputC = std::stoul(block.at("channels"));
			m_BatchSize = std::stoi(trim(block.at("batch"))); // 转换 string to int
         //   assert(m_InputW == m_InputH);
            m_InputSize = m_InputC * m_InputH * m_InputW; // 输入一副图像的像素个数
        }
        else if ((block.at("type") == "region") || (block.at("type") == "yolo"))
        {
			...
			outputTensor.volume = outputTensor.grid_h* outputTensor.grid_w
				*(outputTensor.numBBoxes*(5 + outputTensor.numClasses));//  outputTensor.grid_h* outputTensor.grid_w*outputTensor.numBBoxes为当前的yolo层的检测框个数
